...
export function deepEqualValidator(
  ...
  if (isErrorExpected(expected)) {
    ...
    if (isError(actual)) {
      ...
      return {
        passed: false,
        reason: `Expected error containing "${expected_error}", got "${actual_message}"`,
      };
      ...
      return {
        passed: false,
        reason: `Expected error containing "${expected_error}", got "${actual}"`,
      };
      ...
      return {
        passed: false,
        reason: `Expected error containing "${expected_error}", got "${actual_error}"`,
      };
    }
    return {
      passed: false,
      reason: `Expected error "${expected_error}", got ${JSON.stringify(actual)}`,
    };
  }
