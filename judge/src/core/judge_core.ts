...
export class JudgeCore {
  async run(
    ...
    try {
      if (contract.entry.type === "prototype" && contract.entry.host) {
        fnCode = extractPrototype(
          userCode,
          contract.entry.host,
          contract.entry.name,
        );
      } else if (contract.entry.type === "class") {
        fnCode = extractClass(userCode, problemId);
      } else {
        fnCode = extractExport(userCode, problemId);
      }
    } catch (error) {
      return {
        problemId,
        ...
        cases: [
          {
            ...
            actual: null,
            expected: null,
            error: "Failed to extract code: " + error,
          },
        ],
        duration: 0,
      };
  ...
  private async runTestCase(
    ...
    try {
      ...
      const { actual, meta } = await runInWorkerSandbox(
        contract,
        testCase,
        fnCode,
      );
