export class VirtualClock {
  ...
  private _timers: Map<number, { callback: () => void; time: number; interval?: number }> = new Map()
  ...
  install(): void {
    ...
    globalThis.setInterval = function (callback: () => void, delay = 0): number {
      const id = ++self._idCounter
      self._timers.set(id, {
        callback,
        time: self._now + delay,
  ...
  tick(ms: number): void {
    ...
    while (true) {
      let next_timer: { id: number; time: number; callback: () => void; interval?: number } | null = null

      for (const [id, timer] of this._timers) {
        if (timer.time <= target_time) {
          if (!next_timer || timer.time < next_timer.time) {
            next_timer = { id, time: timer.time, callback: timer.callback, interval: timer.interval }
          }
        }
      }
