...
- 首页采用对齐的 2 x 2 宫格布局，卡片高度一致，视觉风格以暖纸底色、深色正文和棕铜强调色为主。
- 首页入口允许使用多色 emoji 作为识别图标，但整体界面仍需保持克制、整齐和统一。

### 学习页

- 左侧显示题目说明、参数、返回值、思路和参考实现。
- 中间提供带语法高亮的代码编辑器。
- 学习模式首次进入题目时，编辑器默认不填入标准答案，只保留空白起步区。
- 右侧展示基础用例、运行结果和提交反馈。
- `运行` 只执行基础用例。
- `提交` 执行完整用例。
...
- 前端路由必须读取 `BASE_URL` 作为 `BrowserRouter basename`。
- GitHub Pages 必须提供深链刷新兜底，保证 `/learn/:problemId`、`/library/:problemId`、`/theory/:slug` 等路由刷新后不会落到 404。
